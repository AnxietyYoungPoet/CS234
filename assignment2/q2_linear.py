...
class Linear(DQN):
  ...
  def get_q_values_op(self, state, scope, reuse=False):
    ...
    with tf.variable_scope(scope, reuse=reuse):
      out = tf.layers.dense(tf.contrib.layers.flatten(state), num_actions)
    ##############################################################
    ######################## END YOUR CODE #######################
  ...
  def add_update_target_op(self, q_scope, target_q_scope):
    ...
    t_params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope=target_q_scope)
    op_list = [tf.assign(t, q) for t, q in zip(t_params, q_params)]
  ...
  def add_loss_op(self, q, target_q):
    ...
    q_target = self.r + not_done *\
      self.config.gamma * tf.reduce_max(target_q, axis=1)
    q_eval = tf.reduce_sum(tf.one_hot(self.a, num_actions) * q, axis=1)
  ...
  def add_optimizer_op(self, scope):
    ...
    if self.config.grad_clip:
      clip_grads = [(tf.clip_by_norm(gv[0], self.config.clip_val), gv[1]) for gv in grads]
    self.train_op = adam.apply_gradients(clip_grads)
